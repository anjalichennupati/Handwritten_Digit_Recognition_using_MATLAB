function [h, array] = display_network(A, opt_normalize, opt_graycolor, cols, opt_colmajor)
    % A - visualisation matrix
    % reshapes column into a square to form a grid
    warning off all
    
    if ~exist('opt_normalize', 'var') || isempty(opt_normalize)
        opt_normalize= true; % boolean fn
    end
    
    if ~exist('opt_graycolor', 'var') || isempty(opt_graycolor)
        opt_graycolor= true; % boolean fn
    end
    ...
    % h - displayed images are put into an that holds visualisation filters
    % imagesc - diplays the resulting grid
    if opt_graycolor
